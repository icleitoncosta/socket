...
export const USER_JID_FB_SUFFIX = "msgr";
...
export const buildMap = (data: any) => {
  const map = new Map();
...
export const SINGLE_BYTE_TOKEN_MAP = buildMap(SINGLE_BYTE_TOKEN);
export const DICTIONARIES_MAP = DICTIONARIES.map((dict) => buildMap(dict));

export const BIG_ENDIAN_CONTENT = (e: number, t=4) => {
  let r = e;
...
export const xmppSignedPreKey = (key: SignedKeyPair) => {
  return new WapNode('skey', null, [
    new WapNode('id', null, BIG_ENDIAN_CONTENT(key.keyId, 3)),
    new WapNode('value', null, key.keyPair.pubKey),
    new WapNode('signature', null, key.signature),
  ]);
}

export const xmppPreKey = (key: PreKey) => {
  return new WapNode('key', null, [
    new WapNode('id', null, BIG_ENDIAN_CONTENT(key.keyId, 3)),
    new WapNode('value', null, key.keyPair.pubKey),
  ]);
}
...
export const writeRandomPadMax16 = function(e) {
  ...
  (Crypto.webcrypto as any).getRandomValues(t),
  r(e, 1 + (15 & t[0]))
}
...
function r(e, t) {
  ...
  for (var r = 1, a = e.length, i = 0; i < a; i++) r &= e[i] === t[i] ? 1 : 0;
  return 0 !== r;
}

export const areBuffersEqual = (e, t) => {
  return r(new Uint8Array(e), new Uint8Array(t));
};

export const areBytesEqual = r;
export const bytesMod = (e, t) => {
  if (t > 8388607) throw new Error("Divisor is too big");
  for (var r = 0, a = 0; a < e.length; ++a) r = ((r << 8) + e[a]) % t;
  return r;
};

export const randomNumberLessThan = (e) => {
  if (e !== (0 | e)) throw new Error("bound must be int32");
  if (e <= 0) throw new Error("bound must not be positive");
  var t = new Int32Array(1),
    r = e * Math.floor((-1 >>> 1) / e),
    a = -1;
  for (; -1 === a; ) {
    randomHex(t);
    var i = t[0] >>> 1;
...
export const intToBytes = function (e, t) {
  for (var r = t, a = new Uint8Array(e), i = e - 1; i >= 0; i--)
    (a[i] = 255 & r), (r >>>= 8);
  return a;
};

export const uint8ArrayToBuffer = function (e) {
  return e.buffer.slice(e.byteOffset, e.byteLength + e.byteOffset);
};


export const phashV2 = (e: WapJid[]) => {
  for (
    var t = e
        .map((e) =>
          e.toString(),
  ...
  return crypto.subtle
    ...
    .then((e) => {
      return `2:${pHashBase(e.slice(0, 6), 43,47, true)}`
    });
}

function pHashBase(e, t, a, n) {
  var r = 3e3;
  var s = Array.isArray(e) || e instanceof ArrayBuffer ? new Uint8Array(e) : e;
  if (s.length <= r) return pHashBase64I(s, t, a, n);
  for (var o = [], l = 0; l < s.length; l += r)
    o.push(pHashBase64I(s.subarray(l, l + r), t, a, n));
  return o.join('');
}

function pHashBase64I(e, t, r, a) {
  for (
    var i = Math.ceil((4 * e.length) / 3),
      n = 4 * Math.ceil(e.length / 3),
      s = new Array(n),
      o = 0,
      l = 0;
    o < n;
  ...
  for (var u = 0; u < i; u++) {
    var c = s[u];
    s[u] =
      c < 26 ? 65 + c : c < 52 ? 71 + c : c < 62 ? c - 4 : 62 === c ? t : r;
  }
  for (var p = i; p < n; p++) s[p] = 61;
  var f = String.fromCharCode.apply(String, s);
  return a ? f : f.substring(0, i);
}

export const verifyDeviceIdentityAccountSignature = (account: any, identityKeyPair: KeyPair) => {
...
export const generateDeviceSignature = (account: any, identityKeyPair: any) => {
...
export const inflateBuffer = (buffer: any) => new Promise<Buffer>((res) => {
  inflate(buffer, (err, result) => {
      if (err) {
          console.error('err to inflate buffer', err);
          return;
      }
